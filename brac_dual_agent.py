...
class Agent(agent.Agent):
  ...
  def _build_q_loss(self, batch):
    ...
    num_actions = self._action_spec.shape[0]
    id = self._agent_id
    low = id * num_actions
    up = (id + 1) * num_actions
    
    _, a2_p, log_pi_a2_p = self._p_fn(s2)
    agent_action = a2_p - ga2_b[:, low:up]
    if id == 0:
      right = tf.zeros([self._batch_size, 8*num_actions], tf.float32)
      group_action = tf.concat([agent_action, right], axis=-1)
    elif id == 8:
      left = tf.zeros([self._batch_size, 8*num_actions], tf.float32)
      group_action = tf.concat([left, agent_action], axis=-1)
    else:
      right = tf.zeros([self._batch_size, id*num_actions], tf.float32)
      left  = tf.zeros([self._batch_size, (8-id)*num_actions], tf.float32)
      group_action = tf.concat([left, agent_action, right], axis=-1)
    ga2_p = ga2_b + group_action
    
    q2_targets = []
    q1_preds = []
    for q_fn, q_fn_target in self._q_fns:
      q2_target_ = q_fn_target(gs2, ga2_p)
      q1_pred = q_fn(gs1, ga1)
